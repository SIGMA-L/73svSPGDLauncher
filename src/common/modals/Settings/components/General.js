...
const General = () => {
  ...
  useEffect(() => {
    ...
    ipcRenderer
      ...
      .then(appData =>
        fsa
          .readFile(path.join(appData, 'numalabgdlauncher_next', 'rChannel'))
  ...
  const changeDataPath = async () => {
    ...
    const appDataPath = path.join(appData, 'numalabgdlauncher_next');
  ...
  return (
    <>
      ...
      <Content>
        <p>NumaLabGDLauncherの更新するときに使用するリリースを設定します。</p>
        <Select
          css={`
            width: 400px;
          `}
          onChange={async e => {
            ...
            setReleaseChannel(e);
            await fsa.writeFile(
              path.join(appData, 'numalabgdlauncher_next', 'rChannel'),
              e.toString()
            );
          }}
