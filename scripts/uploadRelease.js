...
const main = async () => {
  ...
  try {
    const { data: releasesList } = await axios.default.get(
      `https://api.github.com/repos/SIGMA-L/73svSPGDLauncher/releases`,
      {
        headers: {
      ...
      for (const element in rawChangeLog) {
        if (rawChangeLog[element].length) {
          ...
          for (const e of rawChangeLog[element]) {
            ...
            const advanced =
              ` ([${e?.advanced?.cm}](https://github.com/SIGMA-L/73svSPGDLauncher/commit/${e?.advanced?.cm})` +
              `${
                prSplit
                  ? ` | [#${e?.advanced.pr}](https://github.com/SIGMA-L/73svSPGDLauncher/pull/${prSplit[0]}` +
