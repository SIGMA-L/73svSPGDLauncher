...
const main = async () => {
  ...
  try {
    const { data: releasesList } = await axios.default.get(
      `https://api.github.com/repos/Numa-Lab/NumaLabGDLauncher/releases`,
      {
        headers: {
      ...
      for (const element in rawChangeLog) {
        if (rawChangeLog[element].length) {
          ...
          for (const e of rawChangeLog[element]) {
            ...
            const advanced =
              ` ([${e?.advanced?.cm}](https://github.com/Numa-Lab/NumaLabGDLauncher/commit/${e?.advanced?.cm})` +
              `${
                prSplit
                  ? ` | [#${e?.advanced.pr}](https://github.com/Numa-Lab/NumaLabGDLauncher/pull/${prSplit[0]}` +
